...
{    
    ...
    for (auto gY = 0; gY < gridY; gY++)
    {
        ...
        for (auto gX = 0; gX < gridX; gX++)
        {
            ...
            transformedLabel[(2*numberBodyAndPAFParts+2)*channelOffset + xyOffset] = 1.0;
            //transformedLabel[(2*numberBodyAndPAFParts+3)*channelOffset + xyOffset] = (float(depth_val)-metaData.depthCenterSelf)/1000.0;
            transformedLabel[(2*numberBodyAndPAFParts+3)*channelOffset + xyOffset] = (depth_val>0)?1.0:0.0;
        }
    }
}
